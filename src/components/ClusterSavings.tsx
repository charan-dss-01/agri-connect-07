...
export default function ClusterSavings({ isClusterEligible, originalCost, distance }: ClusterSavingsProps) {
  ...
  return (
    <div className="mt-2 rounded-lg border border-success/30 bg-success/5 p-3 animate-fade-in">
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-[11px] font-semibold text-success">Cluster Eligible • Shared Transport</span>
      </div>
      <div className="grid grid-cols-3 gap-2 text-[10px]">
        <div className="text-center">
          <Truck className="w-3 h-3 mx-auto text-muted-foreground mb-0.5" />
          <p className="text-muted-foreground line-through">₹{originalCost.toLocaleString()}</p>
          <p className="text-[9px] text-muted-foreground">Before</p>
        </div>
        <div className="text-center">
          <Truck className="w-3 h-3 mx-auto text-success mb-0.5" />
          <p className="font-bold text-success">₹{reducedCost.toLocaleString()}</p>
          <p className="text-[9px] text-muted-foreground">After</p>
        </div>
        <div className="text-center">
          <TrendingDown className="w-3 h-3 mx-auto text-primary mb-0.5" />
          <p className="font-bold text-primary">₹{savings.toLocaleString()}</p>
